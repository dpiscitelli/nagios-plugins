...
#  OIDs Indexes
my $nsrpVsdMemberStatus_oid = '.1.3.6.1.4.1.3224.6.2.2.1.3';
...
my %hash_nsrpVsdMemberStatus_status = ( 0 => {	STATE => 'undefined', NAGIOS_STATE => 'CRITICAL' },
										...
										3 => {	STATE => 'inoperable', NAGIOS_STATE => 'CRITICAL' },
);
...
if ( $o_check_cluster ){
	# Get Clusters Members status
	verb("Get Clusters Members status",1,$o_verb);
	my %hash_cluster_members_status = get_table_by_id($session,$nsrpVsdMemberStatus_oid,$o_verb);	
	foreach my $id (keys %hash_cluster_members_status ){
