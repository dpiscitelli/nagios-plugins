...
# ============================== SUMMARY =====================================
#
# Program : check_netstat_status.pl
# Version : 1.0
# Date    : 01/10/2012
...
sub help {
   print <<EOT;
Description:
	Ce plugin remonte les informations reseau en utilisant la commande netstat.
	Par defaut, le plugin remonte l'etat des communications TCP sur le serveur.
	Le plugin analyse le retour de la commande netstat pour les protocoles tcp et udp, en IPv4 et IPv6 

	Gestion des seuils:
	Les seuils sont optionnels. S'ils ne sont pas precises, le plugin remontera toujours
	en OK
	Syntaxe de seuils pour les connexions:
	======================================
	Si "d" correspond a la donnee a controler
	Entier "n" : alerte si d >= n
	Intervalle "n:p" : alerte si d < n ou d > p
	Sous intervalle "n:" : alerte si d < n
	Sous intervalle ":n" : alerte si d > n
	
	Syntaxe des seuils pour les stats:
	==================================
	Les seuils peuvent etre une liste, si on demamnde plusieurs parametres
	Si -t param1,param2,param3
	-w seuilw1,seuilw2,seuilw3
	-c seuilc1,seuilc2,seuilc3
...
-c, --critical
	Le seuil critique pour le plugin. 

-w, --warning
	Le seuil warning pour le plugin. 

-p, --port
	le ou les ports a analyser. 
	Il est possible de passer un port, une liste de ports et aussi de chercher par adresse IP pour un port donne.
	Ex:
	Teste sur le port 22: -p :22
	Teste sur le port 22 et 514: -p :22,:514
	Teste sur le port 22, ouvert sur l'adresse 192.1.1.10: -p 192.1.1.10:22
	
	NB: L'adresse IP est vue comme une expression reguliere. Il est donc possible de faire des recherche sur un reseau entier

-s, --state
	La list des etats de la connexion a analyser: LISTEN|ESTABLISHED
	Ex: --state LISTEN,ESTABLISHED,TIME_WAIT...
	
-D, --distinct
	Pour compter le nombre de connexions venant d'IP differentes	

-R, --remote
	On analyse les connexions du port distant

--netstat-bin
	Pour specifier le chemin de l'executable netstat

--is-listen
	Teste les ports ouverts -> etat LISTEN

--connections
	Teste les connexions -> etat ESTABLISHED
...
sub get_netstat_data {
	...
	foreach $line (@$output) {
		...
		elsif ( ($os eq "LINUX") and ($line =~ m/^(?:tcp|udp)\s+\d+\s+\d+\s+(\S+)\:(\d+)\s+(\S+)\:([*0-9]+)\s+(\w*)/) ) {
			($adr_src,$port_src) = ($1,$2);
			($adr_dest,$port_dest) = ($3,$4);
			if ( $5 ) {
				$state = $5;
...
if ( $os eq "SOL" ){
	...
	$cmd = "$netstat -an --inet && $netstat -an --inet6";
}
elsif ( $os eq "AIX" ){
...
if ( not $o_is_listen and not $o_port){
	if ( $o_connections ) {
		%hash_states_to_catch = get_states_to_catch("ESTABLISHED");
	}
	elsif ( not $o_state ){
		# Get All States
...
if ( $o_is_listen ){
	...
	foreach my $to_analyse ( @list_to_analyse ){
		foreach my $state ( keys %{$to_analyse->{RESULT}} ){
			...
			my $tmp = lc $state;
			my $tmp2 = $to_analyse->{STRING};
			$tmp2 =~ s/^\://;
			$tmp2 =~ s/\:/./g;
			$perfs .= "'tcp_".$tmp2."_".$tmp."'=".$to_analyse->{RESULT}{$state}.";;;; ";	
		}
	}
}
